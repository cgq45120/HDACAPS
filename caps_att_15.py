...
class CapsLayer(object):
        ...
        if self.layer_type == 'CONV':
            ...
            if not self.with_routing:
                capsules = tf.contrib.layers.conv2d(
                    input, self.num_outputs * self.vec_len, self.kernel_size, self.stride, padding="VALID", activation_fn=tf.nn.relu)
                capsules = tf.reshape(
                    capsules, (-1, capsules.shape[1].value*capsules.shape[2].value*self.num_outputs, self.vec_len, 1))
                capsules = self.squash(capsules)
                return (capsules)
        if self.layer_type == 'FC':
            if self.with_routing:
                self.input = tf.reshape(
                    input, shape=(-1, input.shape[1].value, 1, input.shape[-2].value, 1))
                with tf.variable_scope('routing'):
                    b_IJ = tf.constant(np.zeros(
                        [self.batch_size, input.shape[1].value, self.num_outputs, 1, 1], dtype=np.float32))
    ...
    def routing(self, input, b_IJ):
        ...
        W = tf.get_variable('Weight', shape=(1, input_shape[1], self.num_outputs*self.vec_len, input_shape[3],
                                             input_shape[4]), dtype=tf.float32, initializer=tf.random_normal_initializer(stddev=0.01))
        ...
        u_hat = tf.reshape(
            u_hat, shape=[-1, input_shape[1], self.num_outputs, self.vec_len, 1])
        ...
        for r_iter in range(self.iter_routing):
            with tf.variable_scope('iter_' + str(r_iter)):
                ...
                if r_iter == self.iter_routing-1:
                    ...
                    u_produce_v = tf.matmul(
                        u_hat_stopped, v_J_tiled, transpose_a=True)
                    b_IJ += u_produce_v
        return(v_J)
    ...
    def squash(self, vector):
        ...
        scalar_factor = vec_squared_norm / \
            (1 + vec_squared_norm) / tf.sqrt(vec_squared_norm + self.epsilon)
...
class spatial_attention():
    def __init__(self, input_shape):
        ...
        self.b_tanh = self.bias_variable(
            (1, input_shape[1].value, input_shape[2].value, 1))

    def __call__(self, input):
        fc_first = tf.nn.tanh(tf.reduce_sum(tf.multiply(
            input, self.w_tanh), axis=3, keep_dims=True) + self.b_tanh)
        spatial_attention_max = tf.nn.max_pool(fc_first, ksize=[1, 2, 2, 1], strides=[
                                               1, 1, 1, 1], padding='SAME')
        spatial_attention_mean = tf.nn.avg_pool(
            fc_first, ksize=[1, 2, 2, 1], strides=[1, 1, 1, 1], padding='SAME')
        spatial_attention_concat = tf.concat(
            [spatial_attention_max, spatial_attention_mean], axis=3)
        self.attention = tf.contrib.layers.conv2d(
            spatial_attention_concat, num_outputs=1, kernel_size=5, stride=1, padding='SAME', activation_fn=tf.nn.sigmoid)
        output = tf.multiply(input, self.attention)
        return output

    def weight_variable(self, shape):
        initial = tf.truncated_normal(
            shape=shape, stddev=0.01)  # define weight
...
class Capsnet():
    def __init__(self, image_size, channal, num_classes, lambda_val, m_plus, m_minus, epsilon, iter_routing, num_outputs_decode, num_dims_decode, batch_size):
        ...
        self.image = tf.placeholder(
            tf.float32, [self.batch_size, image_size, image_size, channal])
        self.label = tf.placeholder(tf.int64, [self.batch_size, 1])
        label_onehot = tf.one_hot(
            self.label, depth=num_classes, axis=1, dtype=tf.float32)
        ...
        with tf.variable_scope('PrimaryCaps_layer'):
            primaryCaps = CapsLayer(self.batch_size, self.epsilon, self.iter_routing, num_outputs=self.num_outputs_layer_PrimaryCaps,
                                    vec_len=self.num_dims_layer_PrimaryCaps, with_routing=False, layer_type='CONV')
            self.caps1 = primaryCaps(self.attention1, kernel_size=5, stride=1)
        with tf.variable_scope('DigitCaps_layer'):
            digitCaps = CapsLayer(self.batch_size, self.epsilon, self.iter_routing, num_outputs=self.num_outputs_decode,
                                  vec_len=self.num_dims_decode, with_routing=True, layer_type='FC')
            self.caps2 = digitCaps(self.caps1)
        with tf.variable_scope('Masking'):
            self.v_length = tf.sqrt(tf.reduce_sum(
                tf.square(self.caps2), axis=2, keep_dims=True) + self.epsilon)
        ...
        L_c = label_onehot_out * max_l_out + \
            self.lambda_val * (1 - label_onehot_out) * max_r_out
...
class run_main():
    ...
    def two_dimension_graph(self, feature):
        ...
        feature_graph = np.zeros(
            (feature.shape[0], self.image_size, self.image_size, self.channal))
        for i in range(feature.shape[0]):
            for j in range(feature.shape[1]):
                single_use = np.hstack(
                    (feature[i, j, :], 1)).reshape(-1, self.image_size)
                # single_graph = 0.5*np.sqrt(single_use.T*single_use)
                single_graph = self.sigmoid(
                    0.5*np.sqrt(single_use.T*single_use))
    ...
    def train(self, iteration):
        ...
        for step in tqdm(range(iteration), total=iteration, ncols=70, leave=False, unit='b'):
            ...
            for minibatch in minibatches:
                ...
                minibatch_X = minibatch_X.reshape(
                    (-1, self.image_size, self.image_size, self.channal))
                minibatch_Y = minibatch_Y.reshape((-1, 1))
                output_feed = [self.capsnet_model.train_op,
                               self.capsnet_model.total_loss]
                _, loss = self.sess.run(output_feed, feed_dict={
                                        self.capsnet_model.image: minibatch_X, self.capsnet_model.label: minibatch_Y})
    ...
    def save_best(self, accuracy):
        ...
        saver.save(self.sess, 'saver_caps_best/saver' +
                   str(correct_num)+'_caps_'+str(accuracy)+'/muscle.ckpt')
        # saver correct
    ...
    def predict(self):
        ...
        j = 0
        for i in range(num):
            datafortest = self.testData[i*self.batch_size:(i+1)*self.batch_size, :].reshape(
                (-1, self.image_size, self.image_size, self.channal))
            answer = self.sess.run(self.capsnet_model.v_length, feed_dict={
                                   self.capsnet_model.image: datafortest})
            prediction = np.argmax(np.squeeze(answer), axis=1)
            predict_transform = (prediction.reshape(
                (-1, 1)) == self.testFlag[i*self.batch_size:(i+1)*self.batch_size, :])+0
    ...
    def random_mini_batches(self, mini_batch_size=64, seed=0):
        ...
        mini_batch_X = shuffled_X[0 * mini_batch_size: 0 *
                                  mini_batch_size + mini_batch_size, :]
        mini_batch_Y = shuffled_Y[0 * mini_batch_size: 0 *
                                  mini_batch_size + mini_batch_size, :]
