...
class model_cnn(object):
    def __init__(self, image_size,channal, num_classes,learning_rate,max_gradient_norm):
        ...
        first_out_pool = tf.nn.max_pool(first_out, ksize=[1,2,2,1], strides=[1,2,2,1], padding='VALID')
        ...
        second_out_pool = tf.nn.max_pool(second_out, ksize=[1,2,2,1], strides=[1,2,2,1], padding='VALID')

    
        # 全连接
        self.W_fc1 = weight_variable([4*4*128, 1024])
