...
class sae_attention():
    def __init__(self, input_shape):
        ...
        self.w_fuse = self.weight_variable((1, 1, 1, input_shape[3].value))
        self.b_fuse = self.bias_variable((1, 1, 1, 1))

    def __call__(self, input):
        fc_first = tf.nn.tanh(tf.multiply(input, self.w_tanh) + self.b_tanh)
        self.attention = tf.nn.sigmoid(tf.reduce_sum(tf.multiply(fc_first,self.w_fuse), axis=3, keep_dims=True) + self.b_fuse)
...
class Capsnet():
    def __init__(self,image_size,channal,num_classes,lambda_val,m_plus,m_minus,epsilon,iter_routing,num_outputs_decode,num_dims_decode,batch_size):
        ...
        with tf.variable_scope('soft_attention'):
            self.spatialAtt = sae_attention(attention_shape)
...
class run_main():
    ...
    def save_best(self,accuracy):
        ...
        saver.save(self.sess,'saver_caps_sae/saver'+str(self.time_num)+'_caps_'+str(accuracy)+'/muscle.ckpt')
        #saver correct
        m = self.correct_action.shape[0]
        with open('saver_caps_sae/saver'+str(self.time_num)+'_caps_'+str(accuracy)+'/correct.txt','w') as f:
            for i in range(m):
        ...
        with open('saver_caps_sae/saver'+str(self.time_num)+'_caps_'+str(accuracy)+'/epoch_loss.txt','w') as f:
            for i in range(m):
                f.write(str(self.record_epoch_loss[i]))
                f.write('\n')
        #saver loss
        m = self.record_loss.shape[0]
        with open('saver_caps_sae/saver'+str(self.time_num)+'_caps_'+str(accuracy)+'/loss.txt','w') as f:
            for i in range(m):
