...
# heatmap 热力图
def import_data():
    path_file = 'loss_table2.csv'
    # path_file = 'loss_table3.csv'
    data = pd.read_csv(path_file)
    print("the shape of {} is {}".format(path_file, data.shape))
    data = data.set_index('Loss')
    return data
photo = import_data()
#%%    
def plot_line(data):
    ...
    plt.plot(x,data['TDACAPS*'].values,label='TDACAPS*',linewidth='1',linestyle='-',color='m',marker='d',markersize='4')
    ...
    plt.ylabel('Train Loss',fontsize=15,weight='heavy')
    plt.legend(loc=0,ncol=1,fontsize=10)
    plt.tick_params(labelsize=15)
    plt.savefig('loss_table_2.png',dpi=800)
    # plt.savefig('loss_table_3.png',dpi=800)
    plt.show()
plot_line(photo)