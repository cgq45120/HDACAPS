...
class AnnModel():
    def __init__(self, input_size, num_classes,learning_rate,max_gradient_norm):
        W1 = tf.get_variable("W1", [14, 64], initializer=tf.contrib.layers.xavier_initializer())
        b1 = tf.get_variable("b1", [64], initializer=tf.zeros_initializer())
        W2 = tf.get_variable("W2", [64, 32], initializer=tf.contrib.layers.xavier_initializer())
        b2 = tf.get_variable("b2", [32], initializer=tf.zeros_initializer())
        W3 = tf.get_variable("W3", [32, 16], initializer=tf.contrib.layers.xavier_initializer())
        b3 = tf.get_variable("b3", [16], initializer=tf.zeros_initializer())
        W4 = tf.get_variable("W4", [16, num_classes], initializer=tf.contrib.layers.xavier_initializer())
        b4 = tf.get_variable("b4", [num_classes], initializer=tf.zeros_initializer())
        self.global_step = tf.Variable(0, trainable=False)
        self.learning_rate = learning_rate
        self.learning_rate = tf.maximum(tf.train.exponential_decay(learning_rate, self.global_step,5000,0.97,staircase=True),1e-8)
...
class run_main():
    def __init__(self):
        ...
        # self.trainFlag = self.trainFlag - 1 
        # self.testFlag = self.testFlag - 1 
        self.image_size = 14
        self.num_classes = 5
    ...
    def predict(self):
        answer = self.sess.run(self.Ann_model.out_predict, feed_dict={self.Ann_model.input: self.testData})
        prediction = np.argmax(answer,axis=1)
        correct = np.sum((prediction.reshape((-1,1)) == self.testFlag)+0)
        accuracy = correct/self.testFlag.shape[0]
        print(accuracy)
