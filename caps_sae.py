...
class Capsnet():
    def __init__(self, image_size, channal, num_classes, lambda_val, m_plus, m_minus, epsilon, iter_routing, num_outputs_decode, num_dims_decode, batch_size):
        ...
        with tf.variable_scope('Soft_attention'):
            self.spatialAtt = SaeAttention(attention_shape)
...
class RunMain():
    ...
    def save_best(self, accuracy):
        ...
        saver.save(self.sess, 'saver_caps_sae/saver' +str(correct_num)+'_caps_'+str(accuracy)+'/muscle.ckpt')
        # saver correct
        m = self.correct_action.shape[0]
        with open('saver_caps_sae/saver'+str(correct_num)+'_caps_'+str(accuracy)+'/correct.txt', 'w') as f:
            for i in range(m):
        ...
        with open('saver_caps_sae/saver'+str(correct_num)+'_caps_'+str(accuracy)+'/epoch_loss.txt', 'w') as f:
            for i in range(m):
                f.write(str(self.record_epoch_loss[i]))
                f.write('\n')
        # saver loss
        m = self.record_loss.shape[0]
        with open('saver_caps_sae/saver'+str(correct_num)+'_caps_'+str(accuracy)+'/loss.txt', 'w') as f:
            for i in range(m):
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        caps_sae_model = RunMain(data_class)
